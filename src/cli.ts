...
const createTempDir = promisify((callback: (error: any, result: tmp.FileResult) => any) => {
...
export async function buildTypeScript(protoFiles: string[]) {
    console.log('>> GENERATING NODE PROTO INTERFACES >>');
    ...
    try {
        ...
        console.log('Finished transforming the js src');
...
function transformTypeScriptSource(source: string) {
    ...
    source = source.replace(/:(.*)?\((.*)?\|null\);/gm, ': $2;');
    return source;
}
    ...
    ast.find(jscodeshift.FunctionDeclaration).forEach((path) => {
        ...
        if (interfaceComments.length) {
            ...
            path.node.comments.forEach((comment) => {
                ...
                const protoInterface = comment.value.match(/@interface(.*)?/gm);
                console.log('adding message for ', protoInterface ? protoInterface[0] : '');
...
function cleanMethodSignatures(ast: Collection<ASTNode>) {
    ...
    function fixPromiseMethodSignature(path: ASTPath<jscodeshift.ExpressionStatement | jscodeshift.ReturnStatement>) {
        if (!path.node.comments) {
            return;
        }
        let changed = false;
        path.node.comments.forEach((comment) => {
            ...
            if (returnsPromiseRe.test(comment.value)) {
                ...
                comment.value = comment.value.replace(/^([\s*]+@param\s+)(\{.*)$/gm, '$1$2\n$1{grpc.Metadata=} metadata Optional metadata');
            }
        });
        if (changed) {
            path.node.comments = path.node.comments.filter((comment) => comment.value);
